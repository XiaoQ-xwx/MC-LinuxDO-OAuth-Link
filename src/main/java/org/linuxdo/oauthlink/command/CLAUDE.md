[根目录](../../../../../../../CLAUDE.md) > [oauthlink](../) > **command**
...
权限: `oauthlink.command.link`（默认所有人）
...
权限: `oauthlink.admin`（默认 OP）

| 用法 | 说明 |
| :--- | :--- |
| `/oauthlink reload` | 重新加载 config.yml |

## 入口与启动

- 在 `OAuthLink.onEnable()` 中通过 `getCommand("linkld")` 和 `getCommand("oauthlink")` 注册
- `LinkCommand` 实现 `CommandExecutor` + `TabCompleter`
- `OAuthLinkCommand` 仅实现 `CommandExecutor`

## 对外接口

命令通过 Bukkit 命令系统触发，调用 `OAuthLinkService` 方法。

## 关键依赖与配置

- 依赖 `OAuthLinkService`（LinkCommand）或 `JavaPlugin + OAuthLinkService`（OAuthLinkCommand）
...
| `OAuthLinkCommand.java` | `/oauthlink` 管理命令（67 行） |
