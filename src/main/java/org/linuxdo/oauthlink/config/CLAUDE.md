[根目录](../../../../../../../CLAUDE.md) > [oauthlink](../) > **config**
...
- `OAuthConfig.load(plugin)` -- 在 `OAuthLink.onEnable()` 和 `/oauthlink reload` 中调用
...
A: 是的，`/oauthlink reload` 重新调用 `OAuthConfig.load()`，但当前实现中新的 config 对象仅用于验证提示，不会重新注入到已运行的服务中（已知限制）。
