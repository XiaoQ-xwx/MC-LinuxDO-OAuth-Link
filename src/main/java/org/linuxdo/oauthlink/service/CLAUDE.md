[根目录](../../../../../../../CLAUDE.md) > [oauthlink](../) > **service**

# service -- 核心业务编排层

## 模块职责

`OAuthLinkService` 是插件的业务中枢，负责：
- 实现 `OAuthLinkProvider` 接口
...
- 在 `OAuthLink.onEnable()` 中构造，随后调用 `service.onEnable()`
...
| 文件 | 职责 |
| :--- | :--- |
| `OAuthLinkService.java` | 核心业务编排（299 行，本模块唯一文件） |
